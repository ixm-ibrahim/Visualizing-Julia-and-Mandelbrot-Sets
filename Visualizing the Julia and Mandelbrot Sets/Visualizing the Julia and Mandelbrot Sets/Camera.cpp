...
void Camera::ResetSettings()
{
	...
	this->settings.insert({ "moveSpeed", .5 });
...
void Camera::ResetKeyMap()
{
	...
	this->keyMap.insert({ "moveUp", GLFW_KEY_E });
	this->keyMap.insert({ "moveDown", GLFW_KEY_Q });
...
void Camera::Zoom(float distance)
{
	...
	MoveAlongAxis(direction, settings["zoomSpeed"] * distance * (glm::length(position - glm::normalize(position))), false);
}
/*********************************************************************
** Function: Zoom
